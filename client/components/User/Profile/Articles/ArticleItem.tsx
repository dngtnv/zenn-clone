...
const ArticleItem: React.FC<IArticle> = ({ ...article }: IArticle) => {
  return (
    ...
    >
      <Link
        href="#"
        className="absolute left-3 top-3 rounded-[10px] bg-blue-lighter px-[6px] py-[3px] text-center text-[10px] font-semibold uppercase leading-[1.3] text-white"
      >
        {article.articleType}
      </Link>
      <Link href="#" className="flex flex-1 flex-col">
        ...
        <div className="flex-1 pt-[0.8em]">
          <h3 className="line-clamp-3 max-h-[4.55em] text-ellipsis px-[0.9rem] text-base font-bold leading-[1.5]">
            {article.title}
          </h3>
        </div>
        <div className="px-[0.9rem] pb-4 pt-[0.65rem] text-[0.74rem] leading-[1.2]">
